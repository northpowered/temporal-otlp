...
class ServiceWorkflow:
    ...
    async def run(self, payload: str):
        with tracer.start_as_current_span(f"MAIN-{SERVICE_LETTER}-RUN", kind=trace.SpanKind.SERVER) as span:
            ...
            with tracer.start_as_current_span(f"RUN-E-ACTIVITY", kind=trace.SpanKind.CLIENT) as span:
            ...
            with tracer.start_as_current_span(f"RUN-F-ACTIVITY", kind=trace.SpanKind.CLIENT) as span:
